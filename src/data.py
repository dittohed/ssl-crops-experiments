...
class SuperpatchDataset(Dataset):
  ...
  def __init__(
      self, paths: list, spatch_nns: dict,
      input_height: int, input_width: int, patch_size: int, n_agg: int,
      img_transform: Callable, spatch_transform: Callable = None, 
      max_nearest: int = 8, coord_jitter: int = 0, return_whole_images: 
      bool = False
    ):
    ...
    self.spatch_transform = spatch_transform
  ...
  def __getitem__(self, img_idx):
    ...
    if self.spatch_transform is not None:
       spatch1 = self.spatch_transform(spatch1)
       spatch2 = self.spatch_transform(spatch2)
